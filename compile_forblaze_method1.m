...
unsigned char* DecryptBytes(unsigned char* encrypted_bytes, unsigned char* key, int size_encrypted_bytes, int size_key)
{
	...
	//weird error where random bytes are added to the end of decrypted string - this will prevent it.
...
//key here:
unsigned char* key = "\xe1\x77\x51\xca\x97\x22\x00\xe4\x26\xf2\xdd\xf5\x82\x98\x45\xf6";

//size_key here:
int size_key = 16.0;

//stego file location:
NSString *file = [NSString stringWithFormat:@"/tmp/direct_test.jpg"];
